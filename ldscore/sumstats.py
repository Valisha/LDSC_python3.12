...
def _rg(sumstats, args, log, M_annot, ref_ld_cnames, w_ld_cname, i):
    ...
    print("SUMSTATS")
    print(sumstats)
    ## as_matrix function is depecrated and we need to use to_numpy or df.values as an alternative
    # sumstats = sumstats.values
    # to_numpy() doesnot accept the columns attribute so, first choosing the required columns and then converting to a matrix using to_numpy
    ref_ld = sumstats[ref_ld_cnames]
    ref_ld = ref_ld.to_numpy()
